...
def _is_git_cmd(cmd, suffix):
    """Match git commands with or without the Windows -c workaround prefix."""
    return cmd == ["git", *suffix] or cmd == [
        "git",
        "-c",
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(monkeypatch, tmp_path, capsys):
    ...
    def fake_run(cmd, **kwargs):
        recorded.append(cmd)
        if _is_git_cmd(cmd, ["fetch", "origin"]):
            return SimpleNamespace(stdout="", stderr="", returncode=0)
        if _is_git_cmd(cmd, ["rev-parse", "--abbrev-ref", "HEAD"]):
            return SimpleNamespace(stdout="main\n", stderr="", returncode=0)
        if _is_git_cmd(cmd, ["rev-list", "HEAD..origin/main", "--count"]):
            return SimpleNamespace(stdout="1\n", stderr="", returncode=0)
        if _is_git_cmd(cmd, ["pull", "--ff-only", "origin", "main"]):
...
def test_cmd_update_falls_back_to_reset_when_ff_only_fails(monkeypatch, tmp_path, capsys):
    ...
    assert len(reset_calls) == 1
    assert _is_git_cmd(reset_calls[0], ["reset", "--hard", "origin/main"])
